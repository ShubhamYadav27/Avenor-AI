...
class TestCompanyMatching:
    ...
    def test_no_domain_fuzzy_name_match(self):
        ...
        from app.integrations.hubspot.sync import fuzz
...
class TestReconcileCRMAccountsEnumValidation:
    """Verify _reconcile_crm_accounts creates Signals with valid SignalType and SignalSource enum values."""

    def test_reconcile_crm_accounts_uses_valid_enums(self):
        from app.api.routes.companies import _reconcile_crm_accounts
        from app.models import CRMAccountDB, CRMOpportunityDB, SignalType, SignalSource

        ws_id = str(uuid.uuid4())
        acc = CRMAccountDB(
            workspace_id=uuid.UUID(ws_id),
            provider="salesforce",
            external_id="sf-acc-100",
            name="NVIDIA Corporation",
            domain="nvidia.com",
            industry="Technology",
            employee_count=26000,
        )
        opp = CRMOpportunityDB(
            workspace_id=uuid.UUID(ws_id),
            provider="salesforce",
            external_id="sf-opp-200",
            external_account_id="sf-acc-100",
            name="Omniverse Enterprise Migration",
            amount_usd=500000.0,
            stage="Closed Won",
            is_closed_won=True,
        )
        ...
        db.query.return_value.filter_by.side_effect = lambda **kwargs: MagicMock(
            all=lambda: [acc] if "workspace_id" in kwargs and kwargs.get("external_account_id") is None else ([opp] if kwargs.get("external_account_id") == "sf-acc-100" else []),
            first=lambda: None,
        )
        ...
        _reconcile_crm_accounts(db, ws_id)

        # Verify added Signals have valid enum attributes
        from app.models import Signal
        signals = [o for o in added_objects if isinstance(o, Signal)]
        assert len(signals) >= 1
        for sig in signals:
            assert isinstance(sig.signal_type, SignalType)
            assert isinstance(sig.signal_source, SignalSource)
            assert sig.signal_type in list(SignalType)
            assert sig.signal_source in list(SignalSource)
