...
class Workspace(Base):
    ...
    # ── CRM Integration (Phase: CRM-Agnostic Layer) ────────────────────────────
    # Stores the active CRM provider slug for this workspace (e.g. "hubspot", "salesforce").
    # Null means no CRM is connected. Architecture supports multiple in the future.
    crm_provider: Mapped[str | None] = mapped_column(String(50), nullable=True)
    ...
    # Generic CRM connections (supports multiple providers per workspace in future)
    crm_connections: Mapped[list["CRMConnectionDB"]] = relationship(back_populates="workspace")
...
class Company(Base):
    ...
    ai_research: Mapped["CompanyAIResearch"] = relationship(back_populates="company", uselist=False)
    ai_briefings: Mapped[list["CompanyAIBriefing"]] = relationship(back_populates="company")
    ai_sales_coaching: Mapped[list["CompanyAISalesCoaching"]] = relationship(back_populates="company")
...
class CRMUser(Base):
    __tablename__ = 'canonical_crm_users'
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    workspace_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('workspaces.id', ondelete='CASCADE'), nullable=False)
    provider: Mapped[str | None] = mapped_column(String(50))
    external_id: Mapped[str | None] = mapped_column(String(255), index=True)
...
# ── Copilot / Conversation Models (Phase 5.5.1 Foundation) ────────────────────

class CopilotThreadModel(Base):
    __tablename__ = "conversation_threads"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    workspace_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=False, index=True)
    user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("workspace_users.id", ondelete="CASCADE"), nullable=False, index=True)
    title: Mapped[str] = mapped_column(String(255), default="New Strategic Session", nullable=False)
    ...
    messages = relationship("CopilotMessageModel", back_populates="thread", cascade="all, delete-orphan", order_by="CopilotMessageModel.created_at")
    state = relationship("CopilotStateModel", back_populates="thread", uselist=False, cascade="all, delete-orphan")
...
class CopilotMessageModel(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    thread_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("conversation_threads.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    thread = relationship("CopilotThreadModel", back_populates="messages")


class CopilotStateModel(Base):
    __tablename__ = "conversation_state"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    workspace_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=False, index=True)
    thread_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("conversation_threads.id", ondelete="CASCADE"), nullable=False, unique=True, index=True)
    current_company_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("companies.id", ondelete="SET NULL"), nullable=True)
    current_deal_id: Mapped[str | None] = mapped_column(String(255), nullable=True)
    current_contact_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True)
    active_workspace_context: Mapped[dict | None] = mapped_column(JSONB, default=dict)
    active_recommendation: Mapped[dict | None] = mapped_column(JSONB, default=dict)
    active_prompt_context: Mapped[str | None] = mapped_column(Text, nullable=True)
    token_budget: Mapped[int] = mapped_column(Integer, default=8000)
    last_tool_used: Mapped[str | None] = mapped_column(String(100), nullable=True)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)

    thread = relationship("CopilotThreadModel", back_populates="state")
...
class AIEmailStatus(str, enum.Enum):
    PENDING = "pending"
    RUNNING = "running"
    COMPLETED = "completed"
    FAILED = "failed"
...
class AIBriefingStatus(str, enum.Enum):
    PENDING = "pending"
    RUNNING = "running"
    COMPLETED = "completed"
    FAILED = "failed"
...
class AISalesCoachingStatus(str, enum.Enum):
    PENDING = "pending"
    RUNNING = "running"
    COMPLETED = "completed"
    FAILED = "failed"
...
class CompanyAIResearch(Base):
    """
    AI-generated sales research report for a company.

    One row per (workspace, company) — regenerated in place. Generation is
    keyed on `input_hash`, a digest of every piece of Avenor intelligence fed
    to the model. If the hash is unchanged the cached report is returned and
    no LLM call is made; if the underlying intelligence moves, the hash moves
    with it and the report is regenerated on next request.
    """
    __tablename__ = "company_ai_research"
    __table_args__ = (
        UniqueConstraint("workspace_id", "company_id", name="uq_research_workspace_company"),
        Index("ix_research_workspace_company", "workspace_id", "company_id"),
        Index("ix_research_company_hash", "company_id", "input_hash"),
        Index("ix_research_workspace_status", "workspace_id", "status"),
    )
    ...
    # ── Report payload ────────────────────────────────────────
    # summary is prose; every other section is a list of structured objects
    # matching the Pydantic schemas in app/modules/ai/schemas.py.
    summary: Mapped[str | None] = mapped_column(Text)
    buying_signals: Mapped[list] = mapped_column(JSONB, default=list)
    pain_points: Mapped[list] = mapped_column(JSONB, default=list)
    recommended_personas: Mapped[list] = mapped_column(JSONB, default=list)
    outreach_strategy: Mapped[dict] = mapped_column(JSONB, default=dict)
    ...
    # ── Generation metadata ───────────────────────────────────
    status: Mapped[str] = mapped_column(String(20), default=ResearchStatus.PENDING, nullable=False)
    model_provider: Mapped[str | None] = mapped_column(String(50))
    ...
    company: Mapped["Company"] = relationship(back_populates="ai_research")


class CompanyAIEmail(Base):
    """
    AI-generated outbound email derived from the latest AI research.

    Rows are immutable drafts from a sales-workflow perspective: regeneration
    creates a new version, while archiving hides old drafts without deleting
    the audit trail needed for future Gmail/Outlook/CRM send integrations.
    """
    __tablename__ = "company_ai_emails"
    __table_args__ = (
        Index("ix_ai_emails_workspace_company", "workspace_id", "company_id"),
        Index("ix_ai_emails_research_hash", "research_id", "research_input_hash"),
        Index("ix_ai_emails_workspace_status", "workspace_id", "status"),
        Index("ix_ai_emails_contact", "contact_id"),
    )
    ...
    research_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("company_ai_research.id", ondelete="CASCADE"), nullable=False
    )
    ...
    cta_type: Mapped[str] = mapped_column(String(50), nullable=False)
    tone: Mapped[str] = mapped_column(String(50), nullable=False)
    length: Mapped[str] = mapped_column(String(20), nullable=False)
    variation: Mapped[str] = mapped_column(String(1), nullable=False)
    reasoning: Mapped[str | None] = mapped_column(Text)

    status: Mapped[str] = mapped_column(String(20), default=AIEmailStatus.PENDING, nullable=False)
    ...
    prompt_version: Mapped[str | None] = mapped_column(String(50))
    generation_duration_ms: Mapped[int | None] = mapped_column(Integer)
    research_input_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    error_message: Mapped[str | None] = mapped_column(Text)
    ...
    research: Mapped["CompanyAIResearch"] = relationship()
    contact: Mapped["Contact"] = relationship()


class CompanyAIBriefing(Base):
    """
    AI-generated pre-meeting sales briefing derived from existing intelligence.

    Briefings are versioned as append-only rows so earlier meeting prep can be
    preserved, archived, exported, or consumed by later Copilot Chat features.
    Cache validity is keyed to the completed AI research hash plus prompt/model.
    """
    __tablename__ = "company_ai_briefings"
    __table_args__ = (
        Index("ix_ai_briefings_workspace_company", "workspace_id", "company_id"),
        Index("ix_ai_briefings_workspace_status", "workspace_id", "status"),
        Index("ix_ai_briefings_research_hash", "research_id", "input_hash"),
    )
    ...
    source_email_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("company_ai_emails.id", ondelete="SET NULL")
    )

    summary: Mapped[str | None] = mapped_column(Text)
    briefing_json: Mapped[dict] = mapped_column(JSONB, default=dict)
    confidence_score: Mapped[float | None] = mapped_column(Float)

    status: Mapped[str] = mapped_column(String(20), default=AIBriefingStatus.PENDING, nullable=False)
    model_provider: Mapped[str | None] = mapped_column(String(50))
    model_version: Mapped[str | None] = mapped_column(String(100))
    prompt_version: Mapped[str | None] = mapped_column(String(50))
    generation_duration_ms: Mapped[int | None] = mapped_column(Integer)
    ...
    company: Mapped["Company"] = relationship(back_populates="ai_briefings")
    research: Mapped["CompanyAIResearch"] = relationship()
    source_email: Mapped["CompanyAIEmail"] = relationship()


class CompanyAISalesCoaching(Base):
    """
    AI-generated revenue coaching derived from existing Avenor intelligence.

    Rows are append-only and structured for future Copilot Chat, CRM notes,
    manager review and task-generation workflows. Cache validity is keyed to
    the completed research hash, the latest briefing, prompt and model.
    """
    __tablename__ = "company_ai_sales_coaching"
    __table_args__ = (
        Index("ix_ai_sales_coaching_workspace_company", "workspace_id", "company_id"),
        Index("ix_ai_sales_coaching_workspace_status", "workspace_id", "status"),
        Index("ix_ai_sales_coaching_context_hash", "research_id", "briefing_id", "input_hash"),
    )
    ...
    briefing_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("company_ai_briefings.id", ondelete="SET NULL")
    )

    summary: Mapped[str | None] = mapped_column(Text)
    coaching_json: Mapped[dict] = mapped_column(JSONB, default=dict)
    win_probability: Mapped[float | None] = mapped_column(Float)
    confidence_score: Mapped[float | None] = mapped_column(Float)

    status: Mapped[str] = mapped_column(String(20), default=AISalesCoachingStatus.PENDING, nullable=False)
    model_provider: Mapped[str | None] = mapped_column(String(50))
    model_version: Mapped[str | None] = mapped_column(String(100))
    prompt_version: Mapped[str | None] = mapped_column(String(50))
    generation_duration_ms: Mapped[int | None] = mapped_column(Integer)
    ...
    company: Mapped["Company"] = relationship(back_populates="ai_sales_coaching")
    research: Mapped["CompanyAIResearch"] = relationship()
    briefing: Mapped["CompanyAIBriefing"] = relationship()

# ═══════════════════════════════════════════════════════════════
# CRM-Agnostic Integration Layer
# Generic tables that replace all provider-specific CRM tables.
# Provider slug is stored in every row — no separate table per provider.
# ═══════════════════════════════════════════════════════════════

class CRMConnectionDB(Base):
    """
    Generic CRM connection replacing HubSpotConnection.
    One row per (workspace, provider). Stores encrypted OAuth tokens.
    """
    __tablename__ = "crm_connections"
    __table_args__ = (
        UniqueConstraint("workspace_id", "provider", name="uq_crm_conn_workspace_provider"),
        Index("ix_crm_conn_workspace", "workspace_id"),
        Index("ix_crm_conn_provider", "provider"),
    )
    ...
    sync_states: Mapped[list["CRMSyncStateV2"]] = relationship(back_populates="connection")


class CRMSyncStateV2(Base):
    """Generic CRM sync state — per (workspace, provider, object_type)."""
    __tablename__ = "crm_sync_states_v2"
    __table_args__ = (
        UniqueConstraint(
            "workspace_id", "provider", "object_type",
            name="uq_crm_sync_v2_workspace_provider_type"
        ),
        Index("ix_crm_sync_v2_workspace", "workspace_id"),
    )
    ...
    connection_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("crm_connections.id", ondelete="SET NULL")
    )
    provider: Mapped[str] = mapped_column(String(50), nullable=False)
    object_type: Mapped[str] = mapped_column(String(30), nullable=False)
    last_synced_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    total_synced: Mapped[int] = mapped_column(Integer, default=0)
    current_sync_records: Mapped[int] = mapped_column(Integer, default=0)
    lifetime_records_processed: Mapped[int] = mapped_column(Integer, default=0)
    last_run_status: Mapped[str] = mapped_column(String(20), default="pending")
    last_run_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    last_run_created: Mapped[int] = mapped_column(Integer, default=0)
    last_run_updated: Mapped[int] = mapped_column(Integer, default=0)
    last_run_error: Mapped[str | None] = mapped_column(Text)
    historical_import_completed: Mapped[bool] = mapped_column(Boolean, default=False)
    historical_import_completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    historical_records_imported: Mapped[int] = mapped_column(Integer, default=0)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    )

    connection: Mapped["CRMConnectionDB"] = relationship(back_populates="sync_states")


class CRMOpportunityDB(Base):
    """
    Generic CRM Opportunity — provider-agnostic replacement for HubSpotDeal.
    HubSpot Deal / Salesforce Opportunity / Dynamics Opportunity / Zoho Deal.
    """
    __tablename__ = "crm_opportunities"
    __table_args__ = (
        UniqueConstraint("workspace_id", "provider", "external_id", name="uq_crm_opp_workspace_provider_id"),
        Index("ix_crm_opp_workspace_stage", "workspace_id", "stage"),
        Index("ix_crm_opp_company", "company_id"),
        Index("ix_crm_opp_provider", "workspace_id", "provider"),
    )
    ...
    external_account_id: Mapped[str | None] = mapped_column(String(255))
    external_contact_ids: Mapped[list] = mapped_column(JSONB, default=list)
    external_owner_id: Mapped[str | None] = mapped_column(String(255))
    name: Mapped[str | None] = mapped_column(String(500))
    stage: Mapped[str | None] = mapped_column(String(100))
    pipeline: Mapped[str | None] = mapped_column(String(100))
    amount_usd: Mapped[float | None] = mapped_column(Float)
    ...
    avenor_predicted_window: Mapped[str | None] = mapped_column(String(10))
    avenor_first_detected_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    days_ahead_of_crm: Mapped[int | None] = mapped_column(Integer)
    is_historical: Mapped[bool] = mapped_column(Boolean, default=False)
...
class CRMAccountDB(Base):
    """Generic CRM Account — HubSpot Company / Salesforce Account / Dynamics Account / Zoho Account."""
    __tablename__ = "crm_accounts"
    __table_args__ = (
        UniqueConstraint("workspace_id", "provider", "external_id", name="uq_crm_acc_workspace_provider_id"),
        Index("ix_crm_acc_workspace", "workspace_id"),
        Index("ix_crm_acc_domain", "domain"),
    )
...
class CRMContactDB(Base):
    """Generic CRM Contact — HubSpot Contact / Salesforce Contact / Dynamics Contact / Zoho Contact."""
    __tablename__ = "crm_contacts"
    __table_args__ = (
        UniqueConstraint("workspace_id", "provider", "external_id", name="uq_crm_contact_workspace_provider_id"),
        Index("ix_crm_contact_workspace", "workspace_id"),
...
class CRMLeadDB(Base):
    """Generic CRM Lead - Salesforce Lead / Dynamics Lead / Zoho Lead."""
    __tablename__ = "crm_leads"
    __table_args__ = (
        UniqueConstraint("workspace_id", "provider", "external_id", name="uq_crm_lead_workspace_provider_id"),
        Index("ix_crm_lead_workspace", "workspace_id"),
        Index("ix_crm_lead_email", "email"),
        Index("ix_crm_lead_status", "workspace_id", "provider", "status"),
    )
    ...
    converted_account_id: Mapped[str | None] = mapped_column(String(255))
    converted_contact_id: Mapped[str | None] = mapped_column(String(255))
    converted_opportunity_id: Mapped[str | None] = mapped_column(String(255))
    raw_data: Mapped[dict] = mapped_column(JSONB, default=dict)
    synced_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
...
class CRMUserDB(Base):
    """Generic CRM User / Owner — HubSpot Owner / Salesforce User / Dynamics SystemUser / Zoho User."""
    __tablename__ = "crm_users"
    __table_args__ = (
        UniqueConstraint("workspace_id", "provider", "external_id", name="uq_crm_user_workspace_provider_id"),
        Index("ix_crm_user_workspace", "workspace_id"),
    )
...
class CRMAuditLog(Base):
    """Immutable audit trail for all CRM integration operations."""
    __tablename__ = "crm_audit_logs"
    __table_args__ = (
        Index("ix_crm_audit_workspace", "workspace_id", "created_at"),
        Index("ix_crm_audit_provider", "provider", "created_at"),
        Index("ix_crm_audit_operation", "operation"),
    )
...
class Opportunity(Base):
    ...
    external_id: Mapped[str | None] = mapped_column(String(255), index=True)
    external_account_id: Mapped[str | None] = mapped_column(String(255), index=True)
    external_contact_ids: Mapped[list | None] = mapped_column(JSONB)
    external_owner_id: Mapped[str | None] = mapped_column(String(255))
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    stage: Mapped[str | None] = mapped_column(String(100))
    pipeline: Mapped[str | None] = mapped_column(String(100))
