...
def oauth_callback(
    ...
    # Validate state parameter corresponds to a valid, active workspace FIRST
    workspace_id = state
    ...
    if not workspace or not workspace.is_active:
        logger.warning("hubspot_callback_invalid_workspace", workspace_id=workspace_id)
        raise HTTPException(status_code=400, detail="Invalid or inactive workspace state parameter")
    ...
    conn = db.query(HubSpotConnection).filter_by(workspace_id=workspace.id).first()
    if conn is None:
        conn = HubSpotConnection(workspace_id=workspace.id)
        db.add(conn)
...
def disconnect(current_user: CurrentUser, db: Session = Depends(get_db)):
    """Disconnect HubSpot — marks connection inactive and purges synced CRM data for workspace."""
    current_user.require_admin()
    conn = db.query(HubSpotConnection).filter_by(
        workspace_id=current_user.workspace_id
    ).first()
    ...
    from app.crm.routes import purge_crm_data_for_provider
    purge_crm_data_for_provider(db, str(current_user.workspace_id), "hubspot")
