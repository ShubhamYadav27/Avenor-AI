...
class HubSpotClient:
    ...
    def _get(self, path: str, params: dict | None = None, _auth_retried: bool = False) -> dict:
        ...
        try:
            ...
            if resp.status_code == 429:
                ...
                logger.warning(
                    "hubspot_rate_limit_encountered",
                    path=path,
            ...
            if resp.status_code == 401 and not _auth_retried:
                self.retry_count += 1
                logger.info("hubspot_api_401_refreshing_token", path=path, workspace_id=str(self.conn.workspace_id))
                self._refresh_token()
    ...
    def _post(self, path: str, json_body: dict, _auth_retried: bool = False) -> dict:
        """POST with automatic retry on transient errors and rate limits."""
        self.request_count += 1
        try:
            resp = httpx.post(
                f"{HUBSPOT_API_BASE}{path}",
            ...
            if resp.status_code == 401 and not _auth_retried:
                ...
                logger.info("hubspot_api_401_refreshing_token", path=path, workspace_id=str(self.conn.workspace_id))
                self._refresh_token()
                return self._post(path, json_body=json_body, _auth_retried=True)
    ...
    def get_deal_associated_company_ids(self, deal_id: str) -> list[str]:
        ...
        try:
            data = self._get("/crm/v3/associations/deals/companies/batch/read", params={})
