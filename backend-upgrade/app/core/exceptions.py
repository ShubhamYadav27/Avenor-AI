...
class RateLimitError(AvenorError):
    def __init__(self, service: str, retry_after_seconds: int = 60, message: str | None = None):
        msg = message or f"{service} rate limit hit. Retry after {retry_after_seconds}s"
        super().__init__(
            message=msg,
            code="rate_limited",
        )
