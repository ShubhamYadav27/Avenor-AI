"""app/crm/providers/zoho/provider.py"""
...
from app.crm.providers.zoho.client import ZohoAPIClient
...
class ZohoProvider(ICRMProvider):
    def __init__(self, connection=None, db: "Session | None" = None):
        self.connection = connection
        self.db = db
        self._oauth = ZohoOAuth()
        self._webhook = ZohoWebhookHandler()
        self._client: ZohoAPIClient | None = None
    ...
    def _get_client(self) -> ZohoAPIClient:
        if self._client is None:
            if not self.connection or not self.db:
                raise ExternalServiceError("Zoho", "No active connection")
            self._client = ZohoAPIClient(self.connection, self.db)
        return self._client
    ...
    def get_capabilities(self) -> ProviderCapabilities:
        return ProviderCapabilities(
            ...
            webhook_event_types=["insert", "update", "delete"],
            supports_token_refresh=True,
            supports_token_revocation=False,
        )

    def get_oauth_url(self, workspace_id: str) -> OAuthURLResult:
        if not settings.has_zoho:
            raise ExternalServiceError("Zoho", "Zoho CRM credentials not configured")
    ...
    def handle_oauth_callback(self, code: str, state: str, workspace_id: str, db: "Session") -> OAuthCallbackResult:
        ...
        if not workspace:
            raise ExternalServiceError("Zoho", f"Workspace {workspace_id} not found")
        conn = db.query(CRMConnectionDB).filter_by(workspace_id=ws_uuid, provider="zoho").first()
        if conn is None:
            conn = CRMConnectionDB(workspace_id=ws_uuid, provider="zoho")
            db.add(conn)
        ...
        workspace.crm_provider = "zoho"
        db.commit()
        self.connection = conn
        self.db = db
        logger.info("zoho_oauth_callback_complete", workspace_id=workspace_id)
        return result
    ...
    def disconnect(self, workspace_id: str, db: "Session") -> bool:
        ...
        workspace = db.get(Workspace, ws_uuid)
        if workspace and workspace.crm_provider == "zoho":
    ...
    def get_status(self) -> CRMConnectionState:
        if not self.connection:
            return CRMConnectionState(provider="zoho", is_active=False)
        return CRMConnectionState(
            provider="zoho", is_active=self.connection.is_active,
    ...
    def sync_opportunities(self, modified_after: datetime | None = None) -> Generator[CRMOpportunity, None, None]:
        yield from self._get_client().get_deals(modified_after)
    ...
    def health_check(self) -> dict[str, Any]:
        if not self.connection:
            return {"healthy": False, "provider": "zoho", "error": "No active connection"}
        try:
            ...
            return {"healthy": False, "provider": "zoho", "error": str(exc)}
